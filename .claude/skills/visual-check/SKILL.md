...
6. **See the fallback states without any real outage.** The MCP browser has no route interception, but `browser_evaluate` can patch `window.fetch` BEFORE sending a message, so the real UI renders its real error states on demand:
   ...
   - Kill switch / rate limit / length caps: same patch, but resolve a `Response` with status 503/429/413 and a JSON body carrying `code: "disabled"` / `"rate_limited"` / `"too_long"` / `"convo_too_long"` (see `lib/chat-errors.ts` for the contract). Each must render its own copy with NO countdown, and the length and rate-limit codes must render WITHOUT the "learn more in a case study" nudge — that line is gated to real chat failures.
