...
2b. guidance: knowledge-graph refresh (optional local tooling). If the `graphify` CLI is installed and `graphify-out/` exists, run `graphify update .` (AST-only re-extract, no LLM cost) so the graph matches the code being shipped; then, because the graph is published at `/graph.html`, run `graphify export html` and `cp graphify-out/graph.html public/graph.html` so the live page is not stale. Skip silently when graphify is absent — the deploy must never depend on machine-local tooling. No executable gate check exists (graphify is not in CI); tracked in the missing-checks backlog (`CLAUDE.local.md`).
...
3. Link reachability. The agent lists every outbound and internal link from `lib/site.ts` and `lib/projects.ts`; the user opens each and confirms it resolves. Targets:
   ...
   - Contact fallback is `siteConfig.links.linkedin` — there is no email in source (no-PII rule); confirm nothing user-visible promises email contact.
   ...
   - No `#`, TODO, or placeholder href ships. Check: `grep -n "TODO\|href=\"#\"" lib/site.ts lib/projects.ts` comes back empty.
   - `/graph.html` (published codebase knowledge graph, `public/graph.html`) — loads and renders the interactive graph, and its build date is not older than the code being shipped (step 2b refreshes it).
...
5. Mobile and desktop look. The user opens the preview in device mode at 375px (iPhone SE) and 1280px (desktop) and confirms: no horizontal overflow at either width; hero readable and above the fold at 375px; the chat launcher (`ChatWidget` in `app/(main)/layout.tsx`) visible and operable at 375px; interactive targets meet the 44px minimum; navigation usable without hover.
