...
3. Select the model through the failover wrapper, never an inline model id. Provider policy lives in `lib/chat-fallback.ts`:
   - `AI_PROVIDER` (default `gemini`) is the primary; `AI_FALLBACK` (default `anthropic`; `none` disables ALL failover including the OpenAI leg) backs it up; OpenAI joins as a last resort automatically when `OPENAI_API_KEY` is deployed (missing key logs one `[chat] OPENAI_API_KEY missing` error per instance and the chain stays two-deep). Free Gemini by default, pay for Anthropic/OpenAI only when the chain walks down.
   - `orderProviders()` returns the attempt order, skipping any provider the in-memory `createBreaker()` has tripped for its cooldown (default `FALLBACK_COOLDOWN_MS`, 60s; `breaker.trip(provider, ms)` takes a per-trip override) so a known-down provider isn't re-probed every request.
   ...
   - Gemini failover is probe-gated. `onFailover` trips Gemini for a short `PROBE_HOLD_MS` (15s) only; after the response finishes streaming the route runs `probeGemini()` in a `next/server` `after()` callback (never on the response path). Probe `ok` → `breaker.reset("gemini")`, next request returns to Gemini; `quota` (a real 429) → `geminiHealth.quotaExhausted = true` + `breaker.trip("gemini", QUOTA_COOLDOWN_MS)` (5m); `down` → normal 60s cooldown. `geminiHealth` is internal (module state + `[chat] gemini probe: ...` logs), not surfaced to the client.
   ...
   - Truncation is user-visible too: on `finish`, when `finishReason === "length"` the route also stamps `truncated: true` in the same metadata, and `ChatDrawer` renders a "cut short" line under the message instead of ending mid-sentence silently.
   - Chat errors are codes, not prose — the contract is `lib/chat-errors.ts`: guard responses carry `{ code }` (`disabled` / `rate_limited` / `too_long` / `convo_too_long` / `fatal`), stream `onError` returns `retryable` (transient chain failure) or `fatal`, and `ChatDrawer` owns all copy. `too_long` is the per-message cap (`MAX_INPUT_CHARS`, checked against the newest user turn only); `convo_too_long` is the separate whole-transcript cap (`MAX_CONVERSATION_CHARS`) — never sum every message against the per-message cap, that mislabels a normal long chat as an overlong single message. `retryable` drives a VISIBLE countdown (`RETRY_COUNTDOWN_SECONDS`, derived from the default `FALLBACK_COOLDOWN_MS`, so it is a promise, not a guess) plus exactly one auto-retry via `regenerate`; every other code renders honest copy with the LinkedIn/scheduler escape hatches and never promises a retry. The "learn more in a case study" nudge is gated to real chat failures (`retryable`/`fatal`) only — it doesn't fit a length or rate-limit stop.
   Check: `npm run gate` runs `lib/chat-fallback.test.ts` (breaker incl. per-trip cooldown override, ordering, transient/quota classification, wrapper failover incl. 429-failover / 401-no-failover, first-token watchdog, `probeGemini`) + the route handler tests (length guards, post-response probe) + `components/ui/ChatDrawer.test.tsx` for the client half (per-code copy, countdown, nudge gating).
