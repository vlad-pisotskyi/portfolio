...
3. Select the model through the failover wrapper, never an inline model id. Provider policy lives in `lib/chat-fallback.ts`:
   ...
   - Chat errors are codes, not prose — the contract is `lib/chat-errors.ts`: guard responses carry `{ code }` (`disabled` / `rate_limited` / `too_long` / `fatal`), stream `onError` returns `retryable` (transient chain failure) or `fatal`, and `ChatDrawer` owns all copy. `retryable` drives a VISIBLE countdown (equal to the breaker cooldown, so it is a promise, not a guess) plus exactly one auto-retry via `regenerate`; every other code renders honest copy with the LinkedIn/scheduler escape hatches and never promises a retry.
