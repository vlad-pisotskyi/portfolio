...
- **Chat provider failover.** The chat route (`app/api/chat/route.ts`) runs a free primary model with silent same-request failover through a chain of paid fallbacks (up to three providers deep); provider order, the circuit breaker, and transient-error classification live in `lib/chat-fallback.ts`. Defaults `AI_PROVIDER=gemini` → `AI_FALLBACK=anthropic`, with OpenAI as last resort when its key is set. Select models via the failover wrapper — never inline a model id; keep keys in env. See the `chatbot-api` skill.
...
- **E2E:** `npm run test:e2e` — run locally before shipping any user-visible feature. Lives in `e2e/`. Runs axe accessibility scans (home, chat drawer, case-study pages, and light-theme home today; extend per page/flow as they ship).
